...
// Tap / swipe recognizer for the full-screen viewers (#93). Touch only: mouse
// and pen events pass straight through, so desktop behaviour is untouched.
// Returns pointer handlers to spread onto the gesture surface.
export default function useSwipeTap({ onTap, onSwipe }) {
  ...
  function onPointerUp(e) {
    ...
    if (!start || abandonedRef.current || start.id !== e.pointerId) return
    startRef.current = null
