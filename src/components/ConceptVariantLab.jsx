...
function textInputClass() {
  return 'w-full rounded-xs border border-ink-border bg-ink-muted px-3 py-2 text-sm text-cream outline-hidden transition-colors placeholder:text-cream-muted/50 focus:border-cream-muted/50'
}
    ...
    <div className="flex flex-wrap gap-1.5">
      {RATINGS.map((rating) => (
        <button
          ...
          className={`h-8 min-w-8 rounded-xs border px-2 font-mono text-[0.6875rem] transition-colors ${
...
function AddVariantForm({ conceptId, label, onAddVariant, onCancel }) {
  ...
  return (
    <div className="mt-4 space-y-4 rounded-xs border border-ink-border bg-ink-black/35 p-4">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto]">
        ...
        <label className="block md:min-w-48">
          ...
          <input
            ...
            type="file"
            accept="image/*"
            className="block w-full cursor-pointer rounded-xs border border-dashed border-ink-border bg-ink-muted px-3 py-2 text-xs text-cream-muted file:mr-3 file:rounded-xs file:border-0 file:bg-cream/10 file:px-3 file:py-1.5 file:text-xs file:text-cream hover:border-cream-muted/40"
      ...
      <div
        ...
        className={`rounded-xs border-2 border-dashed px-4 py-5 text-center transition-colors ${
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={onCancel}
            className="rounded-xs border border-ink-border px-4 py-2 text-sm text-cream-muted transition-colors hover:border-cream-muted/50 hover:text-cream"
          >
            Cancel
          </button>
          <button
            ...
            disabled={!hasContent}
            onClick={save}
            className="rounded-xs bg-accent px-4 py-2 text-sm text-cream transition-colors hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-30"
          >
            Save Result
          </button>
        </div>
    ...
    <div className="border-t border-ink-border bg-ink-black/25 p-4">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        ...
        <div className="space-y-4">
          <div className="grid gap-3 rounded-xs border border-ink-border bg-ink-muted/20 p-3 sm:grid-cols-2">
            <div>
              <FieldLabel>Provider</FieldLabel>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <button
              ...
              className="rounded-xs border border-ink-border px-3 py-2 text-xs text-cream-muted transition-colors hover:border-accent/60 hover:text-accent"
            >
              Delete
            </button>
          </div>
...
function VariantCard({
  ...
  return (
    <article className="overflow-hidden rounded-xs border border-ink-border bg-ink-black/20">
      <button
        type="button"
      ...
      >
        ...
        <span className="min-w-0 space-y-2">
          <span className="flex flex-wrap items-center gap-2">
            ...
            {variant.isBest && (
              <span className="rounded-xs border border-accent-gold/50 bg-accent-gold/10 px-2 py-0.5 font-mono text-[0.625rem] uppercase tracking-widest text-accent-gold">
                Best
              </span>
            )}
          </span>
...
export default function ConceptVariantLab({
  ...
  return (
    <section className="mt-5 rounded-xs border border-ink-border bg-ink-card/70 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {!isAdding && (
          <button
            ...
            className="rounded-xs bg-accent px-4 py-2 text-sm text-cream transition-colors hover:bg-accent-hover"
          >
            Add Result
          </button>
        )}
      ...
      <div className="mt-4 space-y-2">
        {variants.length === 0 ? (
          <p className="rounded-xs border border-dashed border-ink-border bg-ink-black/20 px-3 py-4 text-sm text-cream-muted">
