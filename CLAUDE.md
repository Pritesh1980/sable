...
- **Every Gemini call sends the key in the `x-goog-api-key` header, never `?key=`** (URLs land in logs and error reports). `src/test/geminiKeyHeader.test.js` scans `src/` for the URL form, so a new caller can't regress it.
