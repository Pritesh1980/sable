...
- **Other GitHub checks**: CodeQL (`codeql.yml`; the bookmarklet's `eval(atob())` alert is
  expected, dismiss it rather than "fix" it), dependency review on PRs, and `pr-labels.yml`
